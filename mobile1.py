...
if data is not None:
    ...
    with c1:
        ...
        df_b = pd.DataFrame([res for s in SECTOR_MAP[top_sec]['stocks'] if (res := analyze(format_ticker(s), data, True))])
    ...
    with c2:
        ...
        df_s = pd.DataFrame([res for s in SECTOR_MAP[bot_sec]['stocks'] if (res := analyze(format_ticker(s), data, False))])
