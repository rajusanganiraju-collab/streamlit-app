...
def highlight_priority(row):
    ...
    if major_conditions >= 2:
        color = '#e6fffa' if day_chg >= 0 else '#fff5f5'
        text = '#008000' if day_chg >= 0 else '#FF0000'
        return [f'background-color: {color}; color: {text}; font-weight: 900'] * len(row)
...
# --- 4a. SEARCH STOCK FEATURE ---
st.markdown("<div style='margin-top: 5px;'></div>", unsafe_allow_html=True)
search_query = st.text_input("🔍 Search Any Stock (e.g. RELIANCE, TCS, ZOMATO):", "").upper().strip()
...
if search_query:
    ...
    if not s_raw.empty:
        ...
        if s_res:
            ...
            c1.metric("LTP", s_res['PRICE'])
            c2.metric("DAY %", f"{s_res['DAY%']}%")
            c3.metric("VOL", s_res['VOL'])
            c4.metric("SCORE", str(s_res['SCORE']))
            s_color = "#008000" if float(s_res['DAY%']) >= 0 else "#FF0000"
            c5.markdown(f"<div style='background-color:{s_color}; color:white; padding:10px; border-radius:5px; text-align:center; font-weight:800; font-size:18px;'>{s_res['STATUS']}</div>", unsafe_allow_html=True)
        else:
...
loading_msg.info("మార్కెట్ డేటా లోడ్ అవుతోంది... ⏳")
data = get_data()
loading_msg.empty()
        ...
        for idx, (ticker, name) in enumerate(INDICES.items()):
            try:
                if ticker in data.columns.levels[0]:
                    ...
                    arrow, txt_color = ("↑", "#008000") if pct >= 0 else ("↓", "#FF0000")
