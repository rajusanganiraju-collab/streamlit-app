...
def analyze(symbol, full_data, check_bullish=True, force=False):
    try:
        ...
        return {"STOCK": f"https://in.tradingview.com/chart/?symbol=NSE:{symbol.replace('.NS','')}", "PRICE": f"{ltp:.2f}", "DAY%": f"{day_chg:.2f}", "STAT": f"{'🚀' if is_bull else '🩸'} {tag} ({time_str})", "CANDLES": int(valid_candles)}
    except: return None

# ⚡ SAFE STYLING: ఎర్రర్ రాకుండా కాలమ్ చెక్ చేస్తుంది
...
if data is not None:
    ...
    if sniper_ticker:
        ...
        if s_res:
            ...
            st.dataframe(pd.DataFrame([s_res]).style.apply(highlight_priority, axis=1), column_config={"STOCK": st.column_config.LinkColumn("STOCK", display_text=r"NSE:(.*)")}, use_container_width=True, hide_index=True)
