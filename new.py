...
st.markdown("""
    ...
    .idx-card { background-color: #0d47a1 !important; border: 1px solid #1976d2; } /* Dark Blue for Nifty/BankNifty */
...
def render_plotly_chart(row, chart_data):
    ...
    # VIX Reverse Logic (Vix going up is red for the market)
    if display_sym == "INDIA VIX":
        color_hex = "#da3633" if row['C'] >= 0 else "#2ea043"
    ...
    try:
        ...
        if not df_chart.empty:
            ...
            if 'Volume' in df_chart.columns and df_chart['Volume'].fillna(0).sum() > 0:
                ...
                df_chart['VWAP'] = df_chart['Typical_Price'].expanding().mean() # For indices without volume
    ...
    if view_mode == "Heat Map":
        ...
        if not df_indices.empty:
            ...
            for _, row in df_indices.iterrows():
                if row['T'] == "INDIA VIX":
                    bg = "bear-card" if row['C'] > 0 else "bull-card" # VIX is reverse
                else:
                    bg = "idx-card" # Blue for Nifty & BankNifty
        ...
        # 1. RENDER INDICES CHARTS FIRST (ROW BY ROW to fix mobile layout)
        if not df_indices.empty:
            idx_list = [row for _, row in df_indices.iterrows()]
            # Loop exactly 3 items at a time
            for i in range(0, len(idx_list), 3):
                cols = st.columns(3)
                for j in range(3):
                    if i + j < len(idx_list):
                        with cols[j]:
                            render_plotly_chart(idx_list[i + j], chart_data)
                            
            # 2. DRAW SEPARATOR LINE IN CHARTS VIEW
            st.markdown("<hr class='custom-hr'>", unsafe_allow_html=True)
        
        # 3. RENDER STOCKS CHARTS (ROW BY ROW)
        if not df_stocks_charts.empty:
            stk_list = [row for _, row in df_stocks_charts.iterrows()]
            for i in range(0, len(stk_list), 3):
                cols = st.columns(3)
                for j in range(3):
                    if i + j < len(stk_list):
                        with cols[j]:
                            render_plotly_chart(stk_list[i + j], chart_data)
