...
def render_swing_terminal_table(df_subset):
    ...
    for i, row in df_sorted.iterrows():
        ...
        row_str += f'<td>{row["P"]:.2f}</td><td class="{day_color}">{row["Day_C"]:.2f}%</td><td>{row["VolX"]:.1f}x</td><td style="font-size:10px;">{custom_status}</td>'
...
def render_highscore_terminal_table(df_subset):
    ...
    for i, row in df_sorted.iterrows():
        ...
        row_str += f'<td style="font-size:10px;">{sec_display}</td>'
        row_str += f'<td>{row["P"]:.2f}</td><td class="{day_color}">{row["Day_C"]:.2f}%</td><td>{row["VolX"]:.1f}x</td><td style="font-size:10px;">{custom_status}</td>'
        ...
        if is_ai: row_str += f'<td style="color:#00BFFF; font-weight:bold;">{int(row["AI_Prob"])}%</td><td style="color:#ffd700;">{int(row["S"])}</td></tr>'
...
def render_chart(row, df_chart, show_pin=True, key_suffix="", timeframe="Intraday (5m)", show_crosshair=False, show_vol=False):
    ...
    if show_pin and display_sym not in ["NIFTY", "BANKNIFTY", "INDIA VIX", "SPX", "DAX", "USD/INR"] and not row.get('Is_Commodity'):
        ...
        if pin_val != is_pinned:
            ...
            st.rerun()
    title_html = f"<a href='{tv_link}' target='_blank' style='color:#ffffff; text-decoration:none; line-height:1.2;'><b>{display_sym}</b><br><span style='font-size:12px;'>₹{row['P']:.2f} <span style='color:{color_hex};'>({sign}{pct_val:.2f}%)</span></span></a>"
    try:
        if not df_chart.empty and 'Low' in df_chart.columns and 'High' in df_chart.columns:
            ...
            def apply_standard_candles(fig_obj, is_subplot):
                ...
                if mask_hv_bear.any():
                    fig_obj.add_trace(go.Candlestick(x=df_chart.index, open=am('Open', mask_hv_bear), high=am('High', mask_hv_bear), low=am('Low', mask_hv_bear), close=am('Close', mask_hv_bear), increasing_line_color='#FF0000', increasing_fillcolor='#FF0000', decreasing_line_color='#FF0000', decreasing_fillcolor='#FF0000', showlegend=False, hoverinfo='skip'), **rc)
            if show_vol:
                ...
                if 'Volume' in df_chart.columns:
                    ...
                    vol_colors = ['rgba(46, 160, 67, 0.4)' if c >= o else 'rgba(218, 54, 51, 0.4)' for c, o in zip(df_chart['Close'], df_chart['Open'])]
                ...
                fig.add_annotation(text=title_html, xref="paper", yref="paper", x=0, xanchor="left", xshift=35, y=0.98, yanchor="top", showarrow=False, font=dict(size=13, color="#ffffff"), bgcolor="rgba(0,0,0,0)")
        ...
        st.markdown(f"<div style='height:150px; text-align:center; color:#888;'>Chart error: {e}</div>", unsafe_allow_html=True)
